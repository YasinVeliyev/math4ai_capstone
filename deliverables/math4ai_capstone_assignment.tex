...
\begin{itemize}
    \item \filepath{data/linear_gaussian.npz} for the linear synthetic task
    \item \filepath{data/moons.npz} for the nonlinear synthetic task
    \item \filepath{data/digits_data.npz} for the digits feature matrix and labels
    \item \filepath{data/digits_split_indices.npz} for the fixed train/validation/test indices
\end{itemize}
...
\begin{itemize}
    \item \textbf{Input representation:} use the flattened 64-dimensional vectors stored in \filepath{data/digits_data.npz}.
    \item \textbf{Labels:} use the digit labels stored in \filepath{data/digits_data.npz}.
    \item \textbf{Split definition:} use \texttt{train\_idx}, \texttt{val\_idx}, and \texttt{test\_idx} from \filepath{data/digits_split_indices.npz}. The script \filepath{scripts/make_digits_split.py} is included for reproducibility; your experiments should use the published indices.
    ...
    \item \textbf{Core preprocessing rule:} outside Track A, use the digits features exactly as provided in \filepath{data/digits_data.npz}.
...
\begin{itemize}
    \item \filepath{data/digits_data.npz},
    \item \filepath{data/digits_split_indices.npz},
    \item \filepath{data/linear_gaussian.npz},
    \item \filepath{data/moons.npz},
    \item \filepath{scripts/make_digits_split.py},
    \item \filepath{scripts/generate_synthetic.py},
    \item the root \filepath{README.md} and \filepath{CHECKLIST.md},
    \item the optional report template \filepath{report/template.tex}.
\end{itemize}
...
Begin with the root \filepath{README.md} and \filepath{CHECKLIST.md}. If you want to inspect how the provided datasets were produced, read \filepath{scripts/generate_synthetic.py} and \filepath{scripts/make_digits_split.py}.
...
Use the digits data in \filepath{data/digits_data.npz} for this track.

\subsection{Track B: Prediction confidence and reliability}

Track B must be conducted on the fixed digits benchmark. Use predictions produced by your trained models on the fixed digits benchmark, using inputs from \filepath{data/digits_data.npz} and the split defined in \filepath{data/digits_split_indices.npz}. This is \textbf{not} a full modern uncertainty-estimation project. Keep the language precise and modest: confidence, reliability, confidence versus correctness, predictive entropy.
...
An optional starting template is included at \filepath{report/template.tex}.
...
Strong reports argue from evidence rather than from preference. If the linear baseline is sufficient for part of the project, say so clearly and justify that claim.
